...
public class MP3Stream : Stream
{
    ...
    public MP3Stream(Stream sourceStream, int chunkSize)
    {
        ...
        Format = ChannelCount switch
        {
            ...
            _ => throw new MP3SharpException(
                $"Unhandled channel count rep: {ChannelCount} (allowed values are 1-mono and 2-stereo)."
            )
        };
    ...
    public override long Position
    {
        ...
        set
        {
            _SourceStream.Position = Math.Clamp(value, 0, _SourceStream.Length);
            IsEOF = false;
            IsEOF |= !ReadFrame();
        }
    }
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        while (true)
        {
            ...
            bytesRead += _Buffer.Read(buffer, offset + bytesRead, count - bytesRead);
