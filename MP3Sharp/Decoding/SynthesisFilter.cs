...
public class SynthesisFilter
{
    ...
    private void compute_pc_samples3(ABuffer buffer)
    {
        ...
        {
            var dp = _d16[i];
            var pcSample = (vp[3 + dvp] * dp[0] + vp[2 + dvp] * dp[1] + vp[1 + dvp] * dp[2] + vp[0 + dvp] * dp[3] +
    ...
    private void compute_pc_samples4(ABuffer buffer)
    {
        ...
        {
            var dp = _d16[i];
            var pcSample = (vp[4 + dvp] * dp[0] + vp[3 + dvp] * dp[1] + vp[2 + dvp] * dp[2] + vp[1 + dvp] * dp[3] +
    ...
    private void compute_pc_samples5(ABuffer buffer)
    {
        ...
        {
            var dp = _d16[i];
            var pcSample = (vp[5 + dvp] * dp[0] + vp[4 + dvp] * dp[1] + vp[3 + dvp] * dp[2] + vp[2 + dvp] * dp[3] +
    ...
    private void compute_pc_samples6(ABuffer buffer)
    {
        ...
        {
            var dp = _d16[i];
            var pcSample = (vp[6 + dvp] * dp[0] + vp[5 + dvp] * dp[1] + vp[4 + dvp] * dp[2] + vp[3 + dvp] * dp[3] +
    ...
    private void Compute_pc_samples15(ABuffer buffer)
    {
        ...
        {
            var dp = _d16[i];
            var pcSample = (vp[15 + dvp] * dp[0] + vp[14 + dvp] * dp[1] + vp[13 + dvp] * dp[2] + vp[12 + dvp] * dp[3] +
