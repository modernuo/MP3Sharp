...
public sealed class Bitstream
{
    ...
    internal bool IsSyncCurrentPosition(int syncmode)
    {
        ...
        var sync = read switch
        {
            0 => true,
            4 => IsSyncMark(headerstring, syncmode, _SyncWord),
            _ => false
        };
    ...
    internal BitstreamException NewBitstreamException(int errorcode) => new(errorcode, null);

    internal BitstreamException NewBitstreamException(int errorcode, Exception throwable) => new(errorcode, throwable);
    ...
    internal int SyncHeader(sbyte syncmode)
    {
        ...
        var headerstring = ((_SyncBuffer[0] << 16) & 0x00FF0000) | ((_SyncBuffer[1] << 8) & 0x0000FF00) |
    ...
    internal bool CheckAndSkipId3Tag(int headerstring)
    {
        ...
        if (id3)
        {
            ...
            // mp3 frame sync.
            var id3_tag_size = 0;
            id3_tag_size |= id3_header[2] & 0x0000007F;
            id3_tag_size <<= 7;
            id3_tag_size |= id3_header[3] & 0x0000007F;
            id3_tag_size <<= 7;
            id3_tag_size |= id3_header[4] & 0x0000007F;
            id3_tag_size <<= 7;
            id3_tag_size |= id3_header[5] & 0x0000007F;

            var id3_tag = new sbyte[id3_tag_size];
